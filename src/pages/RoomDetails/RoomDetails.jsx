    ...
    <Helmet>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="grid md:grid-cols-2 gap-8">
          <img src={room.image} alt={room.title} className="rounded shadow w-full h-auto" />
          <div>
            <h1 className="text-4xl font-bold text-[#236053] mb-4">{room.title}</h1>
            <p className="text-lg text-gray-700 mb-3">{room.description}</p>
            <p className="text-gray-600 mb-2">Price per night: <strong>${room.price}</strong></p>
            <p className="text-gray-600 mb-4">Guests: <strong>{room.capacity || 2}</strong></p>
            ...
            <button onClick={handleBooking} className="btn btn-sm bg-[#236053] text-white hover:bg-[#1b4a42] mt-6">
              Book Now
            </button>
          </div>
        </div>
